...
//            _effectView.backgroundColor=[do_bgColor colorWithAlphaComponent:0.9];
...
//        }
        if(title)
        {
            _titleLableView=[[UILabel alloc]initWithFrame:CGRectMake(0, 20, ScreenWidth, 44)];
        ...
        if(leftId)
        {
            if([leftId isKindOfClass:[NSString class]])
            {
                ...
                [_leftActionView setTitle:tempstr forState:UIControlStateNormal];
                _leftActionView.titleLabel.font=[UIFont systemFontOfSize:14];
                CGFloat width=[NSString getFramByString:tempstr andattributes:@{NSFontAttributeName:_leftActionView.titleLabel.font}].size.width;
                [_leftActionView setTitleEdgeInsets:UIEdgeInsetsMake(0,0, 0,titleWidth-(width+20))];
